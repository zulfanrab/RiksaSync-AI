...
function FollowUpForm({ initialClient, initialEquipment, onClose, onSave }: FollowUpFormProps) {
  ...
  const [eqName, setEqName] = useState(initialEquipment?.equipment_name || '');
  ...
  // Auto-compute due_date when lastDate changes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!eqName.trim()) { setError('Nama Alat wajib diisi.'); return; }
    ...
    try {
      await onSave(
        ...
        { ...(initialEquipment || {}), equipment_name: eqName.trim(), equipment_type: eqType, last_inspection_date: lastDate, due_date: dueDate, certificate_number: certNo }
      );
      onClose();
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto" style={{ maxHeight: 'calc(95vh - 120px)' }}>
          <div className="p-5 space-y-5">
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 block mb-1">Nama Alat *</label>
                  <input value={eqName} onChange={e => setEqName(e.target.value)} placeholder="Forklift 5 Ton Unit-01"
                    className="w-full text-xs border border-slate-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-300 outline-none" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
...
export default function RetentionModule({
  ...
  const handleSaveClientEquipment = async (clientData: Partial<RetentionClient>, eqData: Partial<ClientEquipment>) => {
    if (!isSupabaseConfigured || !supabase) {
      // Local fallback - handled by parent
    ...
    try {
      ...
      if (editingEquipment) {
        const { error } = await supabase.from('client_equipments').update({
          ...
          certificate_number: eqData.certificate_number, updated_at: new Date().toISOString()
        }).eq('id', editingEquipment.id);
        if (error) throw new Error(error.message);
      } else {
        const { error } = await supabase.from('client_equipments').insert([{
          client_id: clientId, equipment_name: eqData.equipment_name, equipment_type: eqData.equipment_type,
          last_inspection_date: eqData.last_inspection_date, due_date: eqData.due_date,
  ...
  const handleAddLog = async (log: Partial<FollowUpLog>) => {
    if (!isSupabaseConfigured || !supabase) return;
    const { error } = await supabase.from('follow_up_logs').insert([log]);
    if (error) throw new Error(error.message);

    // If status is Deal, show deal modal
    if (log.status === 'Deal (Lanjut)' && logModal) {
      setLogModal(null);
      setDealModal({ client: logModal.client, equipment: logModal.equipment });
    }
    await onRefresh();
  };
